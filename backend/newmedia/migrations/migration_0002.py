  ...
  async def Migrate(self, conn: aiosqlite.Connection) -> None:
    ...
    async with conn.execute("SELECT uid, info, blob FROM ImageData") as cursor:
      async for row in cursor:
        uid = row[0]
        prev_info = store_schema.ImageFileV1.FromJSON(bson.loads(row[1]))
        prev_blob = row[2]
        new_info = store_schema.ImageFileV2.FromV1(prev_info)
        ...
        if prev_info.preview_size:
          await conn.execute_insert("INSERT OR REPLACE INTO ImagePreviews(uid, width, height, blob) VALUES(?, ?, ?, ?)", (uid, prev_info.preview_size.width, prev_info.preview_size.height, prev_blob))
        await conn.commit()
